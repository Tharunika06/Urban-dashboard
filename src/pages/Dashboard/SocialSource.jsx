...
import MonthDropdown from "../../components/common/MonthDropdown"; // adjust path if needed

const COLORS = ["url(#gradientColor)", "#f0f0f0"]; // gradient + fallback color
const EMPTY_COLOR = "#E6E8F0"; // gray for empty state
...
const SocialSource = () => {
  ...
  useEffect(() => {
    const fetchBuyersData = async () => {
      try {
        const response = await axios.get("http://192.168.0.152:5000/api/payment/buyers");
  ...
  const totalValue = buyersData[selectedMonth] || 0;
  ...
  return (
    <div className="card text-center">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <MonthDropdown onChange={(month) => setSelectedMonth(month)} />
      </div>
      <span className="text-secondary">
        Total traffic in <strong>{selectedMonth}</strong>
      </span>

      <div className="social-source-chart-container">
        <ResponsiveContainer>
          <PieChart>
            {/* Define gradient only when we have data */}
            {hasData && (
              <defs>
            ...
            >
              {chartData.map((entry, index) => (
                <Cell
                  key={`cell-${entry.name}-${index}`} // ✅ FIX: unique key per slice
