...
const AddProperty = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchOwners = async () => {
      try {
        const res = await axios.get('http://192.168.1.45:5000/api/owners');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.45:5000/api/property', payload, {
        headers: { 
